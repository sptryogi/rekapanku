...
def process_rekap_tiktok(order_details_df, semua_pesanan_df, creator_order_all_df):
    """Fungsi untuk memproses dan membuat sheet 'REKAP' untuk TikTok dengan filter dan logika penghapusan duplikat yang baru."""
    # 1. PERSIAPAN TIPE DATA
    ...
    # >>> BAGIAN BARU 1: FILTER TEGAS PESANAN YANG DIBATALKAN/DIKEMBALIKAN (SEBELUM MERGE) <<<
    if 'CANCELLATION/RETURN TYPE' in semua_pesanan_df.columns:
        # Dapatkan daftar unik ORDER ID yang statusnya 'Cancel' atau 'Return/Refund' dari sumbernya langsung
        cancelled_orders_ids = semua_pesanan_df[
            semua_pesanan_df['CANCELLATION/RETURN TYPE'].isin(['Cancel', 'Return/Refund'])
        ]['ORDER ID'].unique()
    ...
    # Gabungkan data yang sudah bersih dari pesanan batal
    rekap_df = pd.merge(order_details_df, semua_pesanan_df, left_on='ORDER/ADJUSTMENT ID', right_on='ORDER ID', how='right')
    
    # Ekstrak variasi dan bersihkan kolom finansial
    ...
    # >>> BAGIAN BARU 2: HAPUS DUPLIKASI PRODUK SECARA LANGSUNG <<<
    # Hapus baris duplikat berdasarkan kombinasi ID Pesanan, Nama Produk, dan Variasi.
    # Hanya baris pertama yang akan disimpan.
    rekap_df.drop_duplicates(subset=['ORDER ID', 'PRODUCT NAME', 'Variasi'], keep='first', inplace=True)
