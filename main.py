...
def process_rekap(order_df, income_df, seller_conv_df, store_type):
    ...
    if store_type in ['Raka Bookstore', 'Toko Kaliba']:
        ...
        # Biaya Layanan 2% selalu 0 untuk toko ini
        rekap_df['Biaya Layanan 2%'] = 0
...
def process_summary(rekap_df, iklan_final_df, katalog_df, harga_custom_tlj_df, store_type, offline_rows=None):
    ...
    if store_type == "Human Store":
        ...
        mapping_singkatan = {
            ...
            "AL-QUR'AN TERJEMAH  HC AL ALEEM QPP A6 | Jakarta": "Al Aleem A6 QPP",
            "AL-QURAN AL AQEEL SILVER TERMURAH | Jakarta": "Al Aqeel Silver",
            "Paket Wakaf Murah 50 pcs Alquran Al Aqeel | Alquran 18 Baris | Jakarta": "Paket Wakaf Murah Al Aqeel 50 pcs",
            "AL QUR'AN WAQF IBTIDA | AL QUDDUS A5 KERTAS HVS | Jakarta": "Al Quddus A5 HVS",
            "AL QUR'AN AL AQEEL B5 KERTAS HVS | Jakarta": "Al Aqeel B5 HVS",
            "KAMUS BERGAMBAR 3 BAHASA - INDONESIA INGGRIS ARAB | Jakarta": "Kamus Bergambar 3 Bahasa",
            "AL QUR'AN NON TERJEMAH Al AQEEL A5 KERTAS KORAN WAKAF | Jakarta": "AL AQEEL A5 KORAN",
            "Paket Alquran Khusus Wakaf Al Aqeel A5 Kertas Koran | Alquran Murah Kualitas Terbaik Harga Ekonomis | Jakarta": "Al Aqeel A5 Koran",
            "Al QUR'AN NON TERJEMAH AL AQEEL KERTAS KORAN B5 WAKAF | Jakarta": "Al Aqeel B5 Koran",
            "Alquran Cover Emas Kertas HVS Al Aqeel Gold Murah | Jakarta": "Al Aqeel Gold",
            "AL-QUR'AN TERJEMAH HC AL ALEEM A5 | Jakarta": "Al Aleem A5",
            "Komik Pahlawan, Pendidikan Sejarah Untuk Anak | Jakarta": "Komik Pahlawan",
            "AL QUR'AN AL FIKRAH TERJEMAH PER AYAT PER KATA A4 KERTAS HVS | Jakarta": "Al Fikrah A4 HVS",
            "AL QUR'AN HAFALAN SAKU A7 MAHEER KERTAS QPP | Jakarta": "A7 Maheer QPP",
            "AL QUR'AN B5 NON TERJEMAH HVS WARNA PASTEL | Jakarta": "Al Aqeel B5 Pastel",
            "AL QURAN SAKU RESLETING A7 AL QUDDUS KERTAS QPP | Jakarta": "Al Quddus A7 Saku QPP",
            "BUKU CERITA ANAK FABEL SERI DONGENG BINATANG DUA BAHASA | Jakarta": "Fabel Binatang",
            "BUKU CERITA KISAH TELADAN NABI SERI VOL 1-6 | Jakarta": "Kisah Teladan Nabi",
            "AL- QUR'AN TAJWID WARNA WAQF IBTIDA | SUBHAAN A5 KERTAS QPP | Jakarta": "Subhaan A5 QPP",
            "BUKU LAGU HARMONI NUSANTARA LAGU NASIONAL & DAERAH | Jakarta": "Buku Lagu Harmoni Nusantara",
            ...
            "Alquran Cover Emas Kertas HVS Al Aqeel A5 Gold Murah | Jakarta": "Al Aqeel A5 Gold"
        ...
        mapping_singkatan = {
            ...
            "Al-Qur'an Al Aqeel A6 Pastel Kertas HVS 18 Baris | GARUT | Alquran Untuk Wakaf Hadiah Hampers": "Al Aqeel A6 Pastel",
        ...
        mapping_singkatan = {
            "Al-Qur'an Mini Al Aqeel A7 Gold HVS | Cover Metalik | Alquran Souvenir | Semarang": "Al Aqeel A7 Gold",
            "Al-Qur'an Wakaf Al Aqeel A5 Kertas Koran | Mushaf 18 Baris | Semarang": "Al Aqeel A5 Kertas Koran",
...
def process_summary_dama(rekap_df, iklan_final_df, katalog_dama_df, harga_custom_tlj_df, offline_rows=None): # Tambah katalog_dama_df
    ...
    summary_df['Iklan Klik'] = 0.0
    produk_khusus_raw = ["CUSTOM AL QURAN MENGENANG/WAFAT 40/100/1000 HARI", "Alquran Terjemah Faheem A5 Kertas Koran | Alquran Wakaf Hadiah Hampers (BANDUNG)", "Paket Hemat Paket Al Quran | AQ Al Aqeel Wakaf Kerta koran Non Terjemah", "Alquran Al Aqeel A5 Kertas Koran Tanpa Terjemahan Wakaf Ibtida"]
    produk_khusus = [re.sub(r'\s+', ' ', name.replace('\xa0', ' ')).strip() for name in produk_khusus_raw]
    iklan_data = iklan_final_df[iklan_final_df['Nama Iklan'] != 'TOTAL'][['Nama Iklan', 'Biaya']].copy()
...
def process_summary_tiktok(rekap_df, katalog_df, harga_custom_tlj_df, ekspedisi_df, product_data_df, store_choice):
    ...
    if not product_data_df.empty:
        ...
        # Cek apakah kolom yang dibutuhkan ada (menggunakan variabel dinamis)
        if col_name_iklan in product_data_df.columns and col_biaya_iklan in product_data_df.columns:
            # Ambil kolom Biaya dan Nama Produk sesuai variabel
            ads_df = product_data_df[[col_name_iklan, col_biaya_iklan]].copy()
            ...
            # Merge dengan 'outer' menggunakan variabel kolom dinamis
            summary_df = pd.merge(
                summary_df, 
                ads_df, 
                left_on='Nama Produk', 
                right_on=col_name_iklan, # <-- Dinamis
                how='outer'
            )
            
            # Jika Nama Produk kosong (hasil outer merge dari iklan saja), isi dari kolom iklan
            summary_df['Nama Produk'] = summary_df['Nama Produk'].fillna(summary_df[col_name_iklan])
            
            # Hitung Iklan: 
            summary_df['Iklan'] = np.where(
                summary_df['var_count'] > 0,
                summary_df[col_biaya_iklan].fillna(0) / summary_df['var_count'], # <-- Dinamis
                summary_df[col_biaya_iklan].fillna(0) # <-- Dinamis
            )
            
            # Hapus kolom pembantu (menggunakan variabel dinamis)
            summary_df.drop(columns=[col_name_iklan, col_biaya_iklan, 'var_count'], inplace=True, errors='ignore')
        else:
            # Jika kolom tidak ditemukan, set 0
            # st.warning(f"Kolom '{col_name_iklan}' atau '{col_biaya_iklan}' tidak ditemukan di file Iklan TikTok.")
            summary_df['Iklan'] = 0
        ...
        if st.button(button_label):
            ...
            try:
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    ...
                    # Format untuk baris penjualan offline (pink peach)
                    offline_names_format = workbook.add_format({
                        'fg_color': '#FFD1DC',  # Pink peach lembut
                        'border': 1,
                        'align': 'left'
                    })
